...
export class EpisodesService {
  ...
  async createEpisode(
    ...
    const comic = await this.comicsService.findComicById(
      createEpisodeDto.comicId,
    );
    ...
    const last = await this.episodeRepository.findOne({
      where: { comic: { id: createEpisodeDto.comicId } },
      order: { order: 'DESC' },
    });
    ...
    const episode = this.episodeRepository.create({
      ...
      order,
      comic: { id: createEpisodeDto.comicId },
    });
  ...
  async toggleAvailability(
    ...
    if (newAvailability) {
      ...
      await this.episodeNotificationService.notifySubscribersAboutNewEpisode(
        comic,
        episode,
      );
    }
  ...
  async getComicIdByEpisode(
    episodeId: number,
  ): Promise<{ comicId: number; episodeName: string }> {
    const episode = await this.findById(episodeId);
